"""Hard 24 Lateral Powered Study on PixArt-Alpha (DiT + T5-XXL, N=192 Paired Runs).

Strict 2-pass architecture:
Pass 1: Load PixArt, generate all 384 images to disk, measure VRAM, free PixArt.
Pass 2: Load Scorers, score all 384 images from disk, calculate McNemar and stats.
"""
...
STRENGTHS = [0.0, 1.5]
LOG_FILE = ROOT_DIR / "pixart_hard24_study.log"
...
def log_vram(stage: str):
    ...
    log(f"[{stage}] Allocated: {alloc:.2f} GB | Reserved: {res:.2f} GB | Peak: {peak:.2f} GB")
...
def main():
    ...
    t_start = time.time()
    log("=" * 90)
    log("POWERED HARD 24 LATERAL STUDY ON PIXART-ALPHA (N=192 PAIRED RUNS)")
    log("=" * 90)

    device = torch.device("cuda")
    out_dir = ROOT_DIR / "benchmarks" / "images" / "pixart_hard24_n192"
    out_dir.mkdir(parents=True, exist_ok=True)

    # -------------------------------------------------------------
    # PASS 1: GENERATION PASS (PixArt Pipeline ONLY)
    # -------------------------------------------------------------
    log("\n>>> PASS 1: GENERATION PASS (PixArt Pipeline Only)")
    ...
    log_vram("PixArt Loaded")

    # Pre-compute plans and hoist bias tensors once per prompt
    plans = {}
    biases_str15 = {}
    for spec in LATERAL_HARD_24_SPECS:
        ...
        plans[spec["id"]] = plan
        biases_str15[spec["id"]] = build_layout_guidance_bias(
            plan=plan,
    ...
    for str_val in STRENGTHS:
        ...
        for spec in LATERAL_HARD_24_SPECS:
            ...
            if str_val > 0.0:
                bias_t = biases_str15[pid].to(device, dtype=torch.float16)
                for name in cross_attn_names:
                    pipe.transformer.attn_processors[name].set_bias(bias_t)
            else:
                for name in cross_attn_names:
    ...
    gc.collect()
    torch.cuda.empty_cache()
    log("\n[+] PixArt-Alpha pipeline freed.")
    log_vram("Post-Free PixArt")
    ...
    log("\n" + "=" * 115)
    log("PIXART-ALPHA HARD 24 SUMMARY & STATISTICAL BREAKDOWN:")
    log("=" * 115)
    ...
    out_json = ROOT_DIR / "benchmarks" / "pixart_hard24_results.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(payload, f, indent=2)

    total_time = time.time() - t_start
    log(f"\n[+] Powered Hard 24 study on PixArt completed in {total_time:.1f}s ({total_time/60:.2f} min). Saved to {out_json}")
